...
declare -r CKEDITOR_PATCHED_BUILD_TEMP="$(readlink -fm "${CKEDITOR_PATCHED_BUILDER_DIR}/temp")"
### Where the patched and built result shall be copied to.
declare -r CKEDITOR_TARGET="$(readlink -f "${CKEDITOR_MODULE_ROOT}/src/main/resources/META-INF/resources/ckeditor")"
...
function build_ckeditor() {
  ...
  ### Build twice, once with minifying code and once without. Unminified code goes to _source
  ...
  mv "${CKEDITOR_PATCHED_BUILD_OUTPUT}" "${CKEDITOR_PATCHED_BUILD_TEMP}"
  "${CKEDITOR_PATCHED_BUILD}" --build-config "${CKEDITOR_PATCHED_BUILD_CONFIG}" --no-zip --no-tar --debug-level=1
  mv "${CKEDITOR_PATCHED_BUILD_TEMP}" "${CKEDITOR_PATCHED_BUILD_OUTPUT}"/_source
}
