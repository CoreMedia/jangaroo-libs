...
public interface IHistory {
   ...
   * browser's Back button. Equivalent to passing -1 to {@link #go}.
   */
  function back(): void;

  /**
   * This asynchronous method goes to the next page in session history, the same action as when the user clicks the
   * browser's Forward button; this is equivalent to passing 1 to {@link @go}.
   */
   ...
   * effect. Calling {@link #go} without parameters or a value of 0 reloads the current page.
   *
   ...
   *              same result as calling {@link window.location.reload}.
   */
   ...
   *              {@link #pushState}. Whenever the user navigates to the new state, a popstate event is fired, and the
   *              state property of the event contains a copy of the history entry's state object.
   ...
   *              serialized representation is larger than this to {@link #pushState}, the method will throw an
   ...
   *            URL after a call to {@link #pushState}, but it might attempt to load the URL later, for instance after
   *            the user restarts the browser. The new URL does not need to be absolute; if it's relative, it's resolved
   *            relative to the current URL. The new URL must be of the same origin as the current URL; otherwise,
   *            {@link #pushState} will throw an exception. If this parameter isn't specified, it's set to the
   ...
   *                 {@link #replaceState} method. The state object can be null.
   * @param title Most browsers currently ignore this parameter, although they may use it in the future. Passing the
   *              empty string here should be safe against future changes to the method. Alternatively, you could pass a
   ...
   *            {@link #replaceState} throws an exception.
   */
